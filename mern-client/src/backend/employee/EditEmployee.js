...
useEffect(() => {
  ...
  if(myuser)
  {
   history.push(`/backend/editemployee/${id}`);
  }
  else
  {
...
{
    const empdata = await axios.get(`http://localhost:5000/employeeedit/${id}`);
      ...
      <div className="container-fluid">
        <div className="row">
          <div className="col-md-6">
            ...
            e.preventDefault();
            const updateid = `${id}`;
            const employee = {_id:updateid,emp_name,emp_gender,emp_designation,emp_email,emp_contact,company_name ,company_url,company_contact,status}
